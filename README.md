...
├── train_shakespeare_annotated.py
├── train_tinystories.py
├── train_tinystories_annotated.py
...
1. 학습할 때마다 다운로드를 시도하지 않을 수 있음

2. 매번 load_dataset으로 호출하는 경우 데이터 버전이 바뀌면 다운로드에 실패하거나 처리방식이 달라질 수도 있음

3. 데이터를 따로 저장해 놓으면 항상 같은 데이터로 학습할 수 있음
